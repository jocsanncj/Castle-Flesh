...
# Se intenta importar Gestion para poder guardar victorias al final de la partida.
# El try permite probar este archivo aunque gestion.py no esté disponible.
try:
...
# Facciones permitidas dentro del juego y unidades asociadas a cada una.
FACCIONES_VALIDAS = {
...
# Valores principales de economía y balance del juego.
...
# Los Enum ayudan a manejar estados controlados sin depender de textos escritos a mano.
class Fase(Enum):
...
# Esta estructura resume lo que ocurrió durante un combate.
#  (lo llena el módulo de combate del compañero)
@dataclass
...
# Información que se envía a la interfaz cuando termina una ronda.
@dataclass
class ResumenRonda:
    numero:           int
...
# Representación temporal de un jugador dentro de una partida activa.
...
# Clase principal encargada de controlar la partida.
class MotorJuego:
    """
...
# Función auxiliar para crear una partida a partir de los datos del login.
def crear_partida(
    datos_j1: dict,
    datos_j2: dict,
    fn_combate: Callable,
...
# Prueba sencilla para ejecutar este archivo sin abrir la interfaz gráfica.
